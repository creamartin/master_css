...
# PHYS.events$is.multi.actor.event <- 0
# PHYS.events[PHYS.events$number.of.coauthors > 1,]$is.multi.actor.event <- 1

model.avg.rating <- coxph(
  Surv(time = rep(0, dim(bgg.events)[1]), event = bgg.events$IS_OBSERVED) ~
    + event.size
    + exact.repetition
    + sub.rep.1
    + sub.rep.2
